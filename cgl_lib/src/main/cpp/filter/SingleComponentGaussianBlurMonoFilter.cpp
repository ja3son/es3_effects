...
NS_GI_BEGIN
    ...
    SingleComponentGaussianBlurMonoFilter::create(Type type/* = HORIZONTAL*/, int radius/* = 4*/,
                                                  float sigma/* = 2.0*/) {
        SingleComponentGaussianBlurMonoFilter *ret = new(
                std::nothrow) SingleComponentGaussianBlurMonoFilter(type);
    ...
    std::string
    SingleComponentGaussianBlurMonoFilter::_generateOptimizedVertexShaderString(int radius,
                                                                                float sigma) {
    ...
    std::string
    SingleComponentGaussianBlurMonoFilter::_generateOptimizedFragmentShaderString(int radius,
                                                                                  float sigma) {
        if (radius < 1 || sigma <= 0.0) {
        ...
        if (trueNumberOfOptimizedOffsets > numberOfOptimizedOffsets) {
            ...
            for (int i = numberOfOptimizedOffsets; i < trueNumberOfOptimizedOffsets; i++) {
                ...
                shaderStr += str_format(
                        "sum += texture2D(colorMap, blurCoordinates[0] + texelSpacing * %f).r * %f;\n",
