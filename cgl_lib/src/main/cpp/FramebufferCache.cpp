...
NS_GI_BEGIN
    ...
    Framebuffer *
    FramebufferCache::fetchFramebuffer(int width, int height, bool onlyTexture/* = false*/,
        ...
        if (numberOfMatchingFramebuffers < 1) {
            ...
            if (!framebufferFromCache) {
                framebufferFromCache = new Framebuffer(width, height, onlyTexture,
                                                       textureAttributes);
            }
        }
    ...
    std::string FramebufferCache::_getHash(int width, int height, bool onlyTexture,
        ...
        if (onlyTexture) {
            return str_format("%.1dx%.1d-%d:%d:%d:%d:%d:%d:%d-NOFB", width, height,
            ...
            return str_format("%.1dx%.1d-%d:%d:%d:%d:%d:%d:%d", width, height,
                              textureAttributes.minFilter, textureAttributes.magFilter,
                              textureAttributes.wrapS, textureAttributes.wrapT,
                              textureAttributes.internalFormat, textureAttributes.format,
                              textureAttributes.type);
