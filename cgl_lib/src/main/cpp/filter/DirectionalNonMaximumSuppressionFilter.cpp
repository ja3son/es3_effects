...
NS_GI_BEGIN
    ...
    (
            ...
            void main() {
                ...
                vec2 gradientDirection = ((currentGradientAndDirection.gb * 2.0) - 1.0) *
                                         vec2(texelWidth, texelHeight);

                float firstSampledGradientMagnitude = texture2D(colorMap,
                                                                vTexCoord + gradientDirection).r;
                float secondSampledGradientMagnitude = texture2D(colorMap,
                                                                 vTexCoord - gradientDirection).r;

                float multiplier = step(firstSampledGradientMagnitude,
                                        currentGradientAndDirection.r);
                multiplier = multiplier *
                             step(secondSampledGradientMagnitude, currentGradientAndDirection.r);
    ...
    DirectionalNonMaximumSuppressionFilter *DirectionalNonMaximumSuppressionFilter::create() {
        DirectionalNonMaximumSuppressionFilter *ret = new(
                std::nothrow) DirectionalNonMaximumSuppressionFilter();
        if (ret && !ret->init()) {
            delete ret;
            ret = 0;
        }
