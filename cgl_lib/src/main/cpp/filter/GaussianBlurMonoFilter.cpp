...
NS_GI_BEGIN
    ...
    GaussianBlurMonoFilter::create(Type type/* = HORIZONTAL*/, int radius/* = 4*/,
                                   float sigma/* = 2.0*/) {
    ...
    bool GaussianBlurMonoFilter::init(int radius, float sigma) {
        if (Filter::initWithShaderString(_generateOptimizedVertexShaderString(radius, sigma),
    ...
    void GaussianBlurMonoFilter::setSigma(float sigma) {
        ...
        if (_sigma >= 1) // Avoid a divide-by-zero error here
        {
            ...
            calculatedSampleRadius = floor(sqrt(-2.0 * pow(_sigma, 2.0) *
                                                log(minimumWeightToFindEdgeOfSamplingArea *
                                                    sqrt(2.0 * M_PI * pow(_sigma, 2.0)))));
            calculatedSampleRadius += calculatedSampleRadius %
                                      2; // There's nothing to gain from handling odd radius sizes, due to the optimizations I use
        }
    ...
    bool GaussianBlurMonoFilter::proceed(bool bUpdateTargets/* = true*/) {
        ...
        if (rotationSwapsSize(inputRotation)) {
            if (_type == HORIZONTAL) {
                ...
                _filterProgram->setUniformValue("texelHeightOffset",
                                                (float) (1.0 / _framebuffer->getWidth()));
            } else {
                _filterProgram->setUniformValue("texelWidthOffset",
                                                (float) (1.0 / _framebuffer->getHeight()));
    ...
    std::string GaussianBlurMonoFilter::_generateVertexShaderString(int radius, float sigma) {
        ...
        for (int i = 0; i < radius * 2 + 1; ++i) {
            ...
            if (offsetFromCenter == 0) {
                ...
                shaderStr = shaderStr +
                            str_format("blurCoordinates[%d] = texCoord.xy + texelSpacing * (%f);\n",
    ...
    std::string GaussianBlurMonoFilter::_generateFragmentShaderString(int radius, float sigma) {
        ...
        for (int i = 0; i < radius * 2 + 1; ++i) {
            ...
            shaderStr += str_format(
                    "gl_FragColor += texture2D(colorMap, blurCoordinates[%d]) * %f;\n", i,
                    standardGaussianWeights[abs(offsetFromCenter)]);
        }
        shaderStr += "}";
    ...
    std::string
    GaussianBlurMonoFilter::_generateOptimizedVertexShaderString(int radius, float sigma) {
        if (radius < 1 || sigma <= 0.0) {
            return kDefaultVertexShader;
        }
        ...
        for (int i = 0; i < numberOfOptimizedOffsets; ++i) {
            ...
            optimizedGaussianOffsets[i] =
                    (firstWeight * (i * 2 + 1) + secondWeight * (i * 2 + 2)) / optimizedWeight;
        }
    ...
    std::string
    GaussianBlurMonoFilter::_generateOptimizedFragmentShaderString(int radius, float sigma) {
        if (radius < 1 || sigma <= 0.0) {
            return kDefaultFragmentShader;
        }
        ...
        if (trueNumberOfOptimizedOffsets > numberOfOptimizedOffsets) {
            shaderStr += str_format(
                    "highp vec2 texelSpacing = vec2(texelWidthOffset, texelHeightOffset);\n");
            ...
            for (int i = numberOfOptimizedOffsets; i < trueNumberOfOptimizedOffsets; i++) {
                ...
                shaderStr += str_format(
                        "gl_FragColor += texture2D(colorMap, blurCoordinates[0] + texelSpacing * %f) * %f;\n",
