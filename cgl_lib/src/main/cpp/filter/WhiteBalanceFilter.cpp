...
bool WhiteBalanceFilter::init() {
    ...
    registerProperty("temperature", 5000.0,
                     "Adjustment of color temperature (in degrees Kelvin) in terms of what an image was effectively shot in. This means higher Kelvin values will warm the image, while lower values will cool it.",
...
void WhiteBalanceFilter::setTemperature(float temperature) {
    _temperature =
            temperature < 5000 ? 0.0004 * (temperature - 5000.0) : 0.00006 * (temperature - 5000.0);
}
