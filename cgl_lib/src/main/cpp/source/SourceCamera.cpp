...
void SourceCamera::setFrameData(int width, int height, const void *pixels,
                                RotationMode outputRotation/* = RotationMode::NoRotation*/) {
    this->setFramebuffer(0);
    Framebuffer *framebuffer = Context::getInstance()->getFramebufferCache()->fetchFramebuffer(
            width, height, true);
    ...
    CHECK_GL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                          pixels));
#endif
