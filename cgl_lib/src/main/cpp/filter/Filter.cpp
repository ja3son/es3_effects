...
NS_GI_BEGIN
    ...
    Filter::Filter()
            : _filterProgram(0), _filterClassName("") {
    ...
    Filter *Filter::create(const std::string &filterClassName) {
        std::map<std::string, std::function<Filter *()>>::iterator it = _filterFactories.find(
                filterClassName);
    ...
    Filter *Filter::createWithShaderString(const std::string &vertexShaderSource,
    ...
    bool Filter::initWithFragmentShaderString(const std::string &fragmentShaderSource,
                                              int inputNumber/* = 1*/) {
    ...
    bool Filter::proceed(bool bUpdateTargets/* = true*/) {
        ...
        Context::getInstance()->setActiveShaderProgram(_filterProgram);
        _framebuffer->active();
        CHECK_GL(glClearColor(_backgroundColor.r, _backgroundColor.g, _backgroundColor.b,
                              _backgroundColor.a));
        ...
        for (std::map<int, InputFrameBufferInfo>::const_iterator it = _inputFramebuffers.begin();
            ...
            GLuint filterTexCoordAttribute = _filterProgram->getAttribLocation(
                    texIdx == 0 ? "texCoord" : str_format("texCoord%d", texIdx));
    ...
    bool Filter::registerProperty(const std::string &name, int defaultValue,
    ...
    bool Filter::registerProperty(const std::string &name, float defaultValue,
                                  const std::string &comment/* = ""*/,
    ...
    bool Filter::registerProperty(const std::string &name, const std::string &defaultValue,
                                  const std::string &comment/* = ""*/,
    ...
    bool Filter::setProperty(const std::string &name, int value) {
        ...
        if (!rawProperty) {
            ...
            Log("WARNING", "Filter::setProperty The property type is expected to be %s",
