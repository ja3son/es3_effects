...
NS_GI_BEGIN
    ...
    bool FilterGroup::hasFilter(const Filter *filter) const {
        std::vector<Filter *>::const_iterator it = std::find(_filters.begin(), _filters.end(),
                                                             filter);
        if (it != _filters.end())
            return true;
        else
    ...
    Filter *FilterGroup::_predictTerminalFilter(Filter *filter) {
        if (filter->getTargets().size() == 0)
            ...
            return _predictTerminalFilter(
                    dynamic_cast<Filter *>(filter->getTargets().begin()->first));
    }
...
//#endif
    ...
    void FilterGroup::setFramebuffer(Framebuffer *fb,
