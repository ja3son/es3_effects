...
NS_GI_BEGIN
    ...
    (
            ...
            void main() {
                ...
                for (int i = 0; i < GAUSSIAN_SAMPLES; i++) {
                    blurCoordinates[i] =
                            texCoord.xy + texelSpacing * float((i - ((GAUSSIAN_SAMPLES - 1) / 2)));
                }
            }
    );
    ...
    (
            ...
            void main() {
                ...
                distanceFromCentralColor = min(
                        distance(centralColor, sampleColor) * distanceNormalizationFactor, 1.0);
    ...
    BilateralMonoFilter::BilateralMonoFilter(Type type)
            : _type(type), _texelSpacingMultiplier(4.0), _distanceNormalizationFactor(8.0) {
    ...
    bool BilateralMonoFilter::proceed(bool bUpdateTargets/* = true*/) {
        ...
        if (rotationSwapsSize(inputRotation)) {
            if (_type == HORIZONTAL) {
                ...
                _filterProgram->setUniformValue("texelSpacingV", (float) (_texelSpacingMultiplier /
                                                                          _framebuffer->getWidth()));
            } else {
                _filterProgram->setUniformValue("texelSpacingU", (float) (_texelSpacingMultiplier /
    ...
    bool BilateralFilter::init() {
        ...
        registerProperty("texelSpacingMultiplier", 4.0, "The texel spacing multiplier.",
