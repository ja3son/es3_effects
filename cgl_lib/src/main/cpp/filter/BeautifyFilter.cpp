...
NS_GI_BEGIN
    ...
    public:
        ...
        bool init() {
            if (!Filter::initWithFragmentShaderString(kBeautifyCombinationFragmentShaderString,
                                                      3)) {
                return false;
            }
            _intensity = 0.5;
    ...
    BeautifyFilter::BeautifyFilter()
            : _bilateralFilter(0), _cannyEdgeDetectionFilter(0), _combinationFilter(0),
              _hsbFilter(0) {
    }
    ...
    void BeautifyFilter::setInputFramebuffer(Framebuffer *framebuffer,
